...
async function runSetup(guild, interaction = null) {
  // Sauvegarder le salon d'origine AVANT de supprimer quoi que ce soit
  ...
  const editSafe = async (msg) => {
    if (!interaction) return;
    try { await interaction.editReply(msg); } catch {}
  };

  await editSafe('⏳ **Étape 1/4** — Suppression des anciens salons...');

  // 1. Supprimer tous les salons sauf le salon d'origine (pour garder la réponse visible)
  ...
  await editSafe('⏳ **Étape 2/4** — Création des catégories et salons...');
  ...
  await editSafe('⏳ **Étape 3/4** — Publication des explications et règles...');
  ...
  await editSafe('⏳ **Étape 4/4** — Création du message de règlement...');
  ...
  // 5. Répondre AVANT de supprimer le salon d'origine
  await editSafe('✅ **Setup terminé !** Le serveur a été entièrement recréé. Ce salon va être supprimé dans 5 secondes.');
  await dmUser('✅ **Setup Astra RP terminé !** Le serveur a été entièrement recréé avec succès.');

  // 6. Supprimer le salon d'origine maintenant que la réponse est envoyée
  await sleep(5000);
  if (originChannelId) {
    try {
...
client.on('interactionCreate', async interaction => {
  ...
  if (commandName === 'setup') {
    ...
    await interaction.editReply({ content: '⚙️ **Setup en cours...** Cela peut prendre 1 à 2 minutes. Tu recevras un DM à la fin.' });
